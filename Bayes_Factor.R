...
Let's fit a simple [Bayesian linear model], with a
...
to the null . In our case, that means defining a range of
...
that are *complementary*, opposing one-sided hypotheses 
...
**the support interval** as it indicates which values are supported by the data.
 We can do this by once again comparing the prior and
...
of nested as well as non-nested frequentist models .
...
These priors can be formulated as **restricted** priors :
...
**Because these restrictions are on the prior distribution, they are only appropriate for testing pre-planned (*a priori*) hypotheses, and should not be used for any post hoc comparisons .**
...
as we've just seen, both models are practically tied. If this is the case **why limit our estimation of the effect just to one model?**.
...
This section introduces the biased priors obtained when using the common *effects* factor coding (`contr.sum`) or dummy factor coding (`contr.treatment`), and the solution of using orthonormal factor coding (`contr.orthonorm`) . 
